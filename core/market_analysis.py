...
class MarketAnalyzer:
    ...
    def analyze(self, price, candles, tickers=None, options=None):
        ...
        ivs = [float(o.get('implied_volatility') or o.get('iv') or 0) for o in (options or [])]; ivs = [x for x in ivs if x > 0]
        ...
        high_vol = atr_pct >= 1.5 or (ivs and sum(ivs) / len(ivs) >= 75) or abs(funding) >= .001
        if high_vol: regime, confidence = 'HIGH_VOLATILITY_EVENT', min(.95, .6 + atr_pct / 10)
